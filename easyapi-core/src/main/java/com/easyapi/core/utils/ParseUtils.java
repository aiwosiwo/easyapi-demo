...
public class ParseUtils {
    ...
    public static File searchJavaFile(File inJavaFile, String className) {
        ...
        if (file == null && CoreConstants.PAGE_INFO_NAME.equals(className)) {
            file = Utils.getThirdPartJarFile(CoreConstants.PAGE_INFO_FILE);
        }

        // easy-es分页器适配
        if (file == null && CoreConstants.ES_PAGE_INFO_NAME.equals(className)) {
            file = Utils.getThirdPartJarFile(CoreConstants.ES_PAGE_INFO_FILE);
        }

        // 最后的针扎,去目标地址搜索,提供可拓展性,比如有些项目可能用的是自己封装的分页插件,并且打到jar中了,需要开放此能力让这种项目开发者自行适配
        if (file == null) {
            file = getFileFromCurrentProjectPath(className + CoreConstants.JAVA_FILE_SUFFIX);
        }
        ...
        ParseUtils.compilationUnit(modelJavaFile).
                ...
                stream().filter(f -> resultClassName.endsWith(f.getNameAsString())).findFirst().ifPresent(cl -> {
                    ...
                    cl.findAll(FieldDeclaration.class)
                            .stream().filter(fd -> !fd.getModifiers().contains(Modifier.staticModifier()))
                            .forEach(fd -> {
                                ...
                                //忽略字段
                                if (fd.getAnnotationByName(DocIgnore.class.getSimpleName()).isPresent()) {
                                    return;
                                }
    ...
    public static File getFileFromCurrentProjectPath(String fileName) {
        String path = System.getProperty(CoreConstants.USER_DIR) + File.separator + fileName;
